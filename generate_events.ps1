...
class RustStruct {
    ...
    [string] ToString() {
        ...
        if ($this.Description) {
            # Handle multi-line descriptions for struct-level documentation
            $descLines = $this.Description -split "`n"
        ...
        foreach ($field in $this.Fields) {
            ...
            if ($field.Description) {
                # Handle multi-line descriptions for field-level documentation
                $fieldDescLines = $field.Description -split "`n"
                foreach ($line in $fieldDescLines) {
                    $output += "    /// $line"
                }
            }
...
foreach ($groupKey in ($structGroups.Keys | Sort-Object)) {
    ...
    # but merge documentation from all structs
    foreach ($field in $group[0].Struct.Fields) {
        # Collect documentation for this field from all structs in the group
        $fieldDescriptions = @()
        foreach ($structObj in $group) {
            # Find the corresponding field in this struct
            $matchingField = $structObj.Struct.Fields | Where-Object { $_.Name -eq $field.Name }
            if ($matchingField -and $matchingField.Description) {
                ...
                if ($field.Name -eq "retreat") {
                    ...
                    $fieldDescriptions += $matchingField.Description
                }
            }
        }
