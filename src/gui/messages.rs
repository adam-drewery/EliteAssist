...
pub fn messages(state: &State) -> Row<Event> {
    row![
        column![
            ...
            scrollable(column(state.messages
                .iter()
                .map(|item| {
                    row![
                        text(&item.from)
                        ...
                        column![text(&item.text).color(WHITE).size(16)].padding([0, 8]).width(Fill).align_x(Left),
                        text(&item.time).size(12).color(GRAY).width(192).height(16).align_x(Right)
                    ]
                    .padding(2)
                })
                .map(Element::from)
        ...
        .align_x(Left),
        column![
            text("ACTIVITIES").size(20).color(ORANGE),
        ]
        .width(Fill)
        .align_x(Right),
    ]
