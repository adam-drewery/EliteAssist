...
pub fn navigation_bar(state: &State) -> Row<EliteEvent> {
    row![
...
fn navigation_button<'a>(state: &State, title: &'a str, screen: ActiveScreen) -> Column<'a, EliteEvent> {
    
    let style = if discriminant(&state.active_screen) == discriminant(&screen) { selected_style } 
        else { default_style };
    
    let click_event = EliteEvent::NavigateTo(screen);
