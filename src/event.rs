...
/// When written: on a clean shutdown of the game | When written: this player has left a wing | When written: when the player's ship systems shut down (eg in a Thargoid encounter) | When written: when heat exceeds 100% | When Written: when a colonisation beacon is deployed | When written: when the self destruct function is used
#[derive(Clone, Debug, Deserialize)]
pub struct Empty {
...
/// If starting docked in a station | System controlling faction | System controlling faction
#[derive(Clone, Debug, Deserialize)]
...
/// This is logged when a jump is cancelled | At the time the player cancels decommission
#[derive(Clone, Debug, Deserialize)]
pub struct Carrier {
