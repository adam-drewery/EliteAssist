...
/// When written: on a clean shutdown of the game | When Written: when a colonisation beacon is deployed | When written: this player has left a wing | When written: when heat exceeds 100% | When written: when the self destruct function is used | When written: when the player's ship systems shut down (eg in a Thargoid encounter)
#[derive(Clone, Debug, Deserialize)]
pub struct Empty {
