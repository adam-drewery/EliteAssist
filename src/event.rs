...
/// When written: when repairing everything | When Written: when purchasing ammunition
#[derive(Clone, Debug, Deserialize)]
...
/// When Written: when mining fragments are converted unto a unit of cargo by refinery | When written: when scanning some types of data links
#[derive(Clone, Debug, Deserialize)]
...
/// Lists the contents of the ship locker, eg at startup. The full contents are written to a separate file, ShipLocker.json. The full list is also written into the journal at startup (if in a ship) and when boarding a ship. The shiplocker.json file is updated when the locker contents are changed. | Lists the contents of the backpack, in a separate backpack.json file
#[derive(Clone, Debug, Deserialize)]
...
/// When written: when heat exceeds 100% | When Written: when a colonisation beacon is deployed | When written: when the self destruct function is used | When written: this player has left a wing | When written: on a clean shutdown of the game | When written: when the player's ship systems shut down (eg in a Thargoid encounter)
#[derive(Clone, Debug, Deserialize)]
pub struct Empty {
