...
/// When Written: when signing up to a community goal | When written: when opting out of a community goal
#[derive(Clone, Debug, Deserialize)]
...
pub struct CarrierStats {
    ...
    #[serde(rename = "ShipPacks")]
    pub ship_packs: Vec<CarrierStats>,

    #[serde(rename = "ModulePacks")]
    pub module_packs: Vec<CarrierStats>,
...
/// When written: on a clean shutdown of the game | When written: when heat exceeds 100% | When written: when the player's ship systems shut down (eg in a Thargoid encounter) | When Written: when a colonisation beacon is deployed | When written: this player has left a wing | When written: when the self destruct function is used
#[derive(Clone, Debug, Deserialize)]
pub struct Empty {
