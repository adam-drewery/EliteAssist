...
impl Gui {
    pub fn view(state: &State) -> Element<'_, Message> {
        ...
        let dir = journal::get_directory().unwrap_or_else(|_| config::default_journal_dir());
        let has_logs = journal::get_paths(&dir)
            .map(|v| !v.is_empty())
            .unwrap_or(false);
...
fn waiting_spinner<'a>() -> Element<'a, Message> {
    let configured_dir =
        journal::get_directory().unwrap_or_else(|_| config::default_journal_dir());
        ...
        row![
            ...
            column![
                svg(svg::Handle::from_memory(LOADING_PNG))
                    .width(128)
                    .height(128),
                text("The game logs directory is not found at:")
                    .color(ORANGE)
                    .size(16),
                text(configured_dir.display().to_string())
                    .color(ORANGE)
                    .size(16),
                text("Please specify the directory manually")
                    .color(ORANGE)
                    .size(16),
                button(text("Choose directory"))
                    .on_press(Message::Gui(message::Gui::ChooseJournalDir))
            ]
            .align_x(Center),
            column![].width(Fill)
        ],
...
fn loading_bar(state: &State) -> Element<'_, Message> {
    centered![
        ...
        row![
            ...
            column![].width(Fill),
            column![
                text(state.latest_message_timestamp_formatted.as_ref())
                    .color(ORANGE)
                    .size(32)
            ]
        ]
